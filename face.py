'''
method 1: loop through all images (288k+) to find faces and correspond them to spectrogram
method 2: loop through all spectrograms (10k) and correspond them to images then detect faces 

change data cap, confidence value
generator doesn't change too much bc it fools discrim too easily
0.3 loss low adv loss
...
for root, _, files in os.walk(SPECT_DIR):
    for i, file_name in enumerate(files):
        ...
        cat = p[start_index+1:ending_index]

        # add image numbers from spectrogram filename
        idx = p.find('_', ending_index) + 1
        end_idx = p.find('_', idx)
        img_num = p[idx:end_idx]

        # read corresponding image and detect faces
        img_path = os.path.join(PATH, cat, cat, cat) + '_' + img_num + '.JPEG'
        corr_img = plt.imread(img_path, 0)

        # append spectrogram afterwards to exclude black + white
        if corr_img.ndim == 3 and corr_img.shape[2] == 3:
            boxes, conf = m.detect(corr_img)
            # print('before box:', boxes, 'before conf:', conf[0])
            # faces = m.detect_faces(corr_img)

            # if len(faces) != 0:
            if boxes is not None and conf[0] > CONFIDENCE_THRESH:
                print('box:', boxes, 'conf:', conf)
                ...
                #     corr_img = cv2.rectangle(corr_img, (x_left, y_left), (x_right, y_right),
                #     (255, 0, 0), 2)

                corr_img = cv2.resize(corr_img, (IMG_SHAPE[0], IMG_SHAPE[1]), interpolation=cv2.INTER_CUBIC)
                
                
                assert corr_img.shape == IMG_SHAPE
                d1.append(corr_img)
                print(len(d1))
...
for i,img in enumerate(d1):
    d1[i] = normalize_negative_one(img)
...
# load faces model
c_enc = load_model('c_enc_faces.h5')
...
for i in range(NUM_EPOCH):
    ...
    if i % 10 == 0:
        ...
        for i in range(DS):
          for j in range(DS):
              axes[i, j].imshow(cv2.resize(real[count], (IMG_SHAPE[0], IMG_SHAPE[0])))
              axes[i, j].axis('off')
        ...
        for i in range(DS):
            for j in range(DS):
                axes[i, j].imshow(cv2.resize(generated[count], (IMG_SHAPE[0], IMG_SHAPE[0])))
                axes[i, j].axis('off')
